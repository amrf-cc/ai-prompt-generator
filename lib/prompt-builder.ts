...
export function validateRules(r: unknown): asserts r is PromptRules {
  ...
  for (const key of ["nano_banana", "veo", "firefly"]) {
    const t = targets[key];
...
export function buildSystemPrompt(
  ...
  const target = rules.targets[outputTarget];
  ...
  const softwareName =
    software === "photoshop"
      ? "Adobe Photoshop's Generative Fill / Expand"
      : software === "firefly"
        ? "the Adobe Firefly web app"
        : `RunwayML (${targetShort})`;
